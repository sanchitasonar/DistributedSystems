...
int main( int argc, char **argv )
{
	MPI_Comm server;
	MPI_Status status;
	char port_name[MPI_MAX_PORT_NAME],str[50],str1[50],ch;
	int i, tag,again;
	...
	// done sending string to the server tag=1 to end of the string
	MPI_Send(&i, 0, MPI_INT, 0, 1, server);
	// Receive the string from server and display it
	i=0;
	again=1;
	while (again) 
	{
